...
with st.sidebar:
    ...
    font_size_val = st.slider("Cỡ chữ phân tích (px):", 14, 40, 20)

# --- 4. CSS ĐIỀU KHIỂN GIAO DIỆN (ĐẶT SAU SIDEBAR ĐỂ NHẬN BIẾN) ---
st.markdown(f"""
<style>
    ...
    /* Ô PHÂN TÍCH CHÍNH - NHẬN BIẾN TỪ SLIDER */
    .check-box {{ 
        ...
        line-height: 1.7;
        font-size: {font_size_val}px !important;
    }}
    ...
    with st.spinner("🕵️ SmartLens đang thẩm định tư duy..."):
        try:
            ...
            if match:
                ...
                st.session_state.answered_questions = {}
                st.rerun()
        except: st.error("Lỗi AI. Thử lại!")
...
with right:
    st.markdown("### 📝 GHI CHÚ")
    st.text_area("", height=750, key="notes_fixed_v3", placeholder="Ghi chú (750px)...")
...
with left:
    ...
    with tab1:
        ...
        <i>* Mẹo:</i> Nếu báo lỗi, dùng <a href="https://downsub.com/" target="_blank" style="color:#58a6ff; font-weight:bold;">DownSub.com</a> lấy văn bản dán vào tab <b>VĂN BẢN</b>.
        </div>
        """, unsafe_allow_html=True)
