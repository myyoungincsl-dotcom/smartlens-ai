...
# --- 1. CẤU HÌNH AI (CƠ CHẾ TỰ DÒ MODEL) ---
...
# --- 4. HÀM PHÂN TÍCH (SÂU SẮC & VỪA ĐỦ) ---
def run_analysis(text):
    ...
    with st.spinner("🕵️ SmartLens đang thẩm định tư duy chuyên sâu..."):
        try:
            prompt = f"""
            Đóng vai một chuyên gia tư duy phản biện. Phân tích nội dung sau một cách SÂU SẮC, ĐA CHIỀU nhưng NGẮN GỌN (khoảng 250 từ).
            Tập trung vào:
            1. Xác thực: Tính chính xác và tin cậy của luận điểm.
            2. Phản biện: Các góc nhìn đối lập hoặc rủi ro tiềm ẩn.
            3. Mở rộng: Giá trị cốt lõi có thể ứng dụng ngay.
            Dùng <b> và <br> để định dạng. Tạo 3 câu hỏi trắc nghiệm JSON.
            TRẢ VỀ JSON:
            {{
                "verification": "nội dung phân tích sâu sắc",
                "questions": [
            ...
            if match:
                ...
                clean_json = re.sub(r',\s*([\}\]])', r'\1', clean_json) # Fix trailing comma
...
with right:
    st.markdown("### 📝 GHI CHÚ")
    st.text_area("", height=750, key="notes_main", placeholder="Hệ thống ghi chú thông minh (750px)...")

with left:
    tab1, tab2 = st.tabs(["📺 VIDEO YOUTUBE", "📝 VĂN BẢN"])
    
    with tab1:
        st.markdown('<div class="guide-box">💡 <b>Phân tích Video:</b> Dán link YouTube có phụ đề để AI thẩm định lời thoại.</div>', unsafe_allow_html=True)
        ...
        if st.button("🚀 BẮT ĐẦU PHÂN TÍCH"):
            ...
            if v_id:
                try:
                    ...
                    <b>⚠️ Không tìm thấy phụ đề cho video này!</b><br>
                    AI không thể "nghe" video nếu chủ sở hữu tắt phụ đề. Để tiếp tục:<br>
                    1. Copy phần <b>Mô tả video</b> hoặc <b>Bình luận chính</b> của video.<br>
                    2. Chuyển sang Tab <b>📝 VĂN BẢN</b> bên cạnh.<br>
                    3. Dán vào và nhấn Thẩm định.
                    </div>
                    """, unsafe_allow_html=True)
            else: st.error("Link video không hợp lệ!")
    
    with tab2:
        txt = st.text_area("Dán nội dung cần thẩm định chuyên sâu:", height=250, key="txt_input_final")
        ...
        for i, q in enumerate(st.session_state.current_data.get('questions', [])):
            ...
            if ans:
                if ans.startswith(q['correct']):
                    ...
                    st.error("❌ Chưa đúng. Hãy phân tích kỹ nội dung phía trên!")
