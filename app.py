...
def run_analysis(text, title="Bài học"):
    ...
    # Cắt ngắn 3000 ký tự để AI phản hồi nhanh, tránh lỗi 504 Deadline Exceeded
    clean_text = text[:3000]
    with st.spinner("SmartLens AI đang mổ xẻ dữ liệu..."):
        try:
            prompt = f"""
            Hãy đóng vai chuyên gia thẩm định. Phân tích nội dung sau bằng tiếng Việt:
            1. Xác thực (Sự thật). 2. Phản biện (Góc nhìn khác). 3. Mở rộng (Ứng dụng).
            Yêu cầu: Viết nội dung phân tích rõ ràng, dùng <b> và <br> để định dạng.
            ...
            st.error(f"Lỗi AI: {e}. Hãy thử lại với văn bản ngắn hơn.")
